          ...
          {whtaWeDoTaxationData.map(({ id, icon, title, desc }, idx) => (
            <div className="col-span-12 sm:col-span-6 md:col-span-4" key={id}>
              <StaggerEffect id={idx} key={id}>
                <div
                  className=" bg-white text-black rounded-xl shadow-xl p-6 xl:p-8 flex flex-col border border-white group hover:border-mainTextColor duration-700 hover:bg-s2 group items-start justify-start"
                  key={id}
                >
                  <div className="text-4xl text-green-800  p-3 group-hover:text-mainTextColor duration-500 bg-softBg1 group-hover:bg-white rounded-fulls">
                    {icon}
                  </div>
