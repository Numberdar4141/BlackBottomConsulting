    ...
    <header>
      <div
        className={` top-0 left-0 right-0 z-50 ${
          scrolled
            ? "animationOne fixed bg-black shadow-md"
            : "animationTwo bg-black absolute"
        }`}
      >
        <div className="flex justify-between items-center container lg:text-[20px] text-white ">
          <div className="pb-1 flex justify-start items-center gap-3">
            ...
            <Link to={"/"}>
              <img src={logo} className="w-[180px]" alt="accupay logo" />
            </Link>
          </div>
              ...
              {headerMenu.map((menu) => (
                <li key={menu.id} className="">
                  {menu.isSubmenu ? (
                    ...
                    >
                      <div
                        className={`flex justify-center items-center gap-1 relative px-2 py-3 rounded-lg  ${
                          isMenuActive(menu) ? "" : ""
                        }`}
                      >
                        {menu.name}
                      ...
                      <ul
                        className={`absolute top-12 left-0 pointer-events-none  flex justify-start items-start flex-col w-[200px] py-6 gap-3 opacity-0 invisible group-hover:visible group-hover:opacity-100 group-hover:z-50 bg-s1 text-white/80  rounded-lg ${
                          menuToggle === menu.id
                      ...
                      >
                        {menu.submenu?.map(({ id, name, link }) => (
                          <li key={id}>
                            <Link
                              to={link}
                              className={`px-6 hover:ml-2 duration-500  ${
                                path.pathname === link ? "" : ""
                              }`}
                            >
                              {name}
                    ...
                    <Link
                      to={menu.link!}
                      className={` duration-700 px-2 py-3 rounded-lg ${
                        path.pathname === menu.link ? "" : ""
                      }`}
                    >
                      {menu.name}
                    </Link>
